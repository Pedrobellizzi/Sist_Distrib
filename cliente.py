...
class ClienteChat: 
    ...
    def descobrir_procedimento(self, procedure_name):                 # Descobrir e localizar os procedimentos do chat através do Binder
        sucesso, procedimento = self.binder.lookup_procedure(procedure_name)
        if not sucesso:
            print(procedimento)
            return False
        address_Server = f"http://{procedimento['endereco']}:{procedimento['porta']}/"
    ...
    def executar(self):                    # Executar as funçionalidades do cliente
        print("Descobrindo procedimentos do chat...")
        if not self.descobrir_procedimento("ChatService"):
            return
